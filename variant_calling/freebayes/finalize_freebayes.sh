...
if [ ${#@} -lt 5 ]; then
    echo "Usage: bash finalize_freebayes.sh [source_dir] [output_dir] [scratch_dir] [hours] [memory] (chromosome:yes)"
    exit 1
fi
...
do
    ...
    job_prefix=FB_finalize_${isolate_tag_map}
    ...
    do
        ...
        ### STEP 1: split any multi-allelic sites and normalize indels;
        ...
        echo "bcftools norm --multiallelics -any -f ${ref_file} ${vcf_filepath} -Ov -o ${MAsplit_norm_file}" >> $job_file
        ...
        echo "SnpSift extractFields -e '' -s ';' ${vcfwave_MAsplit_norm_file} \
            POS REF ID ALT AF AO SAF SAR DP QUAL MQM ORIGIN 'GEN[0].GL' > ${initial_variants_file}" >> $job_file
        
        echo "echo -e 'POS\tREF\tID\tALT\tAF\tAO\tAO_F\tAO_R\tDP\tQUAL\tMQM\tORIGIN\tGL' > ${final_variants_file}" >> $job_file
