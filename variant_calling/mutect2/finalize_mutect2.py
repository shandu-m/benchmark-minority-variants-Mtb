...
def finalize_mutect2(variants_file):
    ...
    filtered_variants_df = variants_df[(variants_df.AC >= 2) & (variants_df.AF >= 0.01)].copy(deep=True).reset_index(drop=True)

    ### label if variant came from a complex type/MNP ###
    complex_type = 1 - filtered_variants_df.ORIGIN.isna().astype(int)
    filtered_variants_df["MT_COMPLEX_TYPE"] = complex_type

    ### split the SB field for MA sites ###
    dup_pos = filtered_variants_df[filtered_variants_df.POS.duplicated()]["POS"].values
    
    nondup_pos_df = filtered_variants_df[~filtered_variants_df.POS.isin(dup_pos)].copy(deep=True).reset_index(drop=True)
    
    dup_pos_df = filtered_variants_df[filtered_variants_df.POS.isin(dup_pos)].copy(deep=True).reset_index(drop=True)

    prev_pos = np.nan
    skip_pos = False
    for i in dup_pos_df.index:

        pos = dup_pos_df.loc[i, "POS"]

        if not skip_pos:
    
            if pos != prev_pos:
                prev_pos = pos
                num_dups = list(dup_pos_df.POS.values).count(pos)
                
                skip_pos = False
    
                pos_counter = 1
    
                SB = dup_pos_df.loc[i, "SB"]
                if SB.count("|") != num_dups:
                    skip_pos = True

                else:
                    new_SB = "|".join([SB.split("|")[0], SB.split("|")[pos_counter]])
                    dup_pos_df.loc[i, "SB"] = new_SB
    ...
    variants_df = pd.concat([nondup_pos_df, dup_pos_df]).sort_values(['POS']).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-v", "--variantFile", dest='variants_file', type=str, required=True, help='TSV file with variants pulled from a normalized Mutect2 VCF to be finalized.')
    
    cmd_line_args = parser.parse_args()
    
    variants_file = cmd_line_args.variants_file

    finalize_mutect2(variants_file)