...
def finalize_vardict(variants_file):
    ...
    ACs = variants_df.AC.values

    variants_df = variants_df[(variants_df.AC >= 2) & (variants_df.AF >= 0.01)]
    
    ### label if variant came from a complex type/MNP ###
    complex_type = 1 - variants_df.ORIGIN.isna().astype(int)
    variants_df["VD_COMPLEX_TYPE"] = complex_type
...
if __name__ == "__main__":
    ...
    parser.add_argument("-v", "--variantFile", dest='variants_file', type=str, required=True, help='TSV file with variants pulled from a normalized VarDict VCF to be finalized.')
    
    cmd_line_args = parser.parse_args()
    
    variants_file = cmd_line_args.variants_file

    finalize_vardict(variants_file)