...
if [ ${#@} -lt 5 ]; then
    echo "Usage: bash finalize_lofreq.sh [source_dir] [output_dir] [scratch_dir] [hours] [memory]"
    exit 1
fi
...
do
    ...
    job_prefix=LF_finalize_${isolate_tag_map}
    ...
    do
        ...
        ### STEP 1: extract necessary info with SNPsift and write to tsv file ###

        initial_variants_file=${temp_dir}/${tag}.temp.${tool}.tsv
        final_variants_file=${output_dir}/${tag}.${tool}.tsv

        echo "SnpSift extractFields -e '' -s ';' ${vcf_filepath} \
            POS REF ALT QUAL DP AF > ${initial_variants_file}" >> $job_file
        
        echo "echo -e 'POS\tREF\tALT\tQUAL\tDP\tAF' > ${final_variants_file}" >> $job_file
        echo "tail -n+2 ${initial_variants_file} >> ${final_variants_file}" >> $job_file

        ### STEP 2: finalize variants list ###
