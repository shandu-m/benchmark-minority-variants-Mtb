...
if [ ${#@} -lt 11 ]
then
    printf "Usage: bash simulate_data.sh [multifasta_dir] [abundance_dir] [output_dir] [scratch_dir] [n_reads] [memory] [hours] [queue] [error_model] [coverage] [label]\n"
...
do
    ...
    do
        ...
        wc_grep=$(grep "$coverage,$freq" /home/sm624/projects/mixed_calls/benchmarking/comprehensive/allowed_combinations.csv | wc -l | awk '{print $1}')
        if [ $wc_grep -eq 0 ]
        then
            continue
        fi
